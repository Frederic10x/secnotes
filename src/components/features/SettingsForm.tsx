            ...
            {THEME_OPTIONS.map(({ value, label, icon }) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 border rounded-lg p-3 flex items-center gap-2 cursor-pointer text-sm transition-colors duration-150 ${
                    selected
            ...
            {FONT_OPTIONS.map(({ value, label, size }) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 border rounded-lg p-3 flex items-center gap-2 cursor-pointer transition-colors duration-150 ${
                    selected
                      ? 'border-accent bg-accent/10 text-accent'
                      : 'border-border text-muted hover:border-muted'
                  }`}
        ...
        <div className="flex flex-col gap-2">
          {SHORTCUTS.map(({ keys, label }) => (
            <div key={keys} className="flex items-center justify-between py-1">
              ...
              <kbd className="bg-background border border-border rounded px-2 py-0.5 font-mono text-xs text-text">
                {keys}
              </kbd>
            </div>
          ))}
